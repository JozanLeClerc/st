...
void set_notifmode(int type, KeySym ksym) {
	...
	if ( ksym == -1 ) {
		...
		memcpy(g, TLINE(bot), col * sizeof(Glyph));

	}
	else if ( ksym == -2 )
		memcpy(TLINE(bot), g, col * sizeof(Glyph));

	if ( type < 2 ) {
		char *z = lib[type];
		for (deb = &TLINE(bot)[col - 6], fin = &TLINE(bot)[col]; deb < fin; z++, deb++)
		...
		for (deb = &TLINE(bot)[0], fin = &TLINE(bot)[col]; deb < fin; deb++)
			deb->mode = ATTR_REVERSE,
				deb->u = ' ',
				deb->fg = defaultfg, deb->bg = defaultbg;
		TLINE(bot)[0].u = ksym;
...
void select_or_drawcursor(int selectsearch_mode, int type) {
	...
	xdrawcursor(term.c.x, term.c.y, TLINE(term.c.y)[term.c.x],
			 term.ocx, term.ocy, TLINE(term.ocy)[term.ocx]);
}
...
void search(int selectsearch_mode, Rune *target, int ptarget, int incr, int type, TCursor *cu) {
	...
	for (i = term.col * term.c.y + term.c.x + incr; i != bound; i += incr) {
		for (r = target; r - target < ptarget; r++) {
			if ( *r == TLINE((i + r - target) / term.col)[(i + r - target) % term.col].u ) {
...
int trt_kbdselect(KeySym ksym, char *buf, int len) {
	...
	if ( selectsearch_mode & 2 ) {
		if ( ksym == XK_Return ) {
			...
			TLINE(term.bot)[ptarget].u = target[ptarget - 1];
		}
